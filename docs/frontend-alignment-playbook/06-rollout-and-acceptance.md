...
### App 闭环
`app-register -> app-list submit -> audit-center/app-audit approve -> publish -> app-market resolve/open -> invoke`（无 grant 步）
...
| `provider-list` | 列表可达；Provider 元数据 **不**承担 per-resource invoke Grant（已删） |
...
- [x] 改造顺序覆盖资源中心/审核中心/（~~授权中心~~ 已删）市场/治理/壳层/SDK沙箱
- [x] 至少三条端到端验收路径（App/MCP/调用）完整
- [x] 三角色验收口径完整
- [x] 回归检查项完整
- [x] 逐角色页面回归脚本完整

...
- 统一资源主链路：
  ...
  - SDK：`/sdk/v1/*`
  - 沙箱：`/sandbox/*`
  - ~~逐资源调用授权~~：`/resource-grants*`、`/grant-applications*` **~~已废弃（2026-04-09 下线）~~**；替代方案：API Key + Scope + `published` 状态
...
| `ResourceGrantManagementPage` | ~~资源授权列表、新增授权、撤销授权~~ | — | ~~已废弃（2026-04-09 下线）~~；替代方案：API Key + Scope + `published` 状态 |
...
| `resource-grant-management` | `user:manage`（~~已废弃（2026-04-09 下线）~~；替代方案：API Key + Scope + `published` 状态；建议移除菜单项） |
...
|---|---|---|---|---|
| `resourceGrantService.*` | — | `/resource-grants*` | **~~已废弃（2026-04-09 下线）~~** | 替代方案：API Key + Scope + `published` 状态；前端应移除调用。个人设置 `GET .../api-keys/.../resource-grants` 仅占位 `[]`。 |
...
| `resourceGrantService.create` | DatasetMarket | ~~已废弃（2026-04-09 下线）~~；替代方案：API Key + Scope + `published` 状态 |

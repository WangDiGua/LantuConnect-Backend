...
**前端接线状态（持续更新）**：公告、授权审批、入驻申请、审计日志等列表筛选已透传服务端 query；**敏感词** 见 **[前端补全与后端分工](./frontend-completion-before-backend.md)**（keyword + 编辑弹窗）。**2026-03-30** 后端已补：监控 `call-logs`/`alerts` 分页筛选（`status`、`severity`、`alertStatus`）、`GET /grant-applications/mine` 的 `keyword`/`q`、审计 `result`、**`GET /user-mgmt/tokens` + `PATCH .../revoke`**、资源授权列表 `keyword`。详见 **[功能缺口矩阵](./frontend-feature-gap-matrix.md)**。仅后端升级而不同步前端时，仍可能表现为检索无效。
...
| 2026-03-30 | 后端对齐落地：监控分页 `status`/`severity`/`alertStatus`；`mine` keyword；审计 `result`；`/user-mgmt/tokens` 与撤销；`/resource-grants` `keyword`。 |

...
| ~~**资源授权** `ResourceGrantManagementPage`~~ | **已下线**（后端无 `/resource-grants*`） | 前端宜移除该页或改静态说明 |

**说明**：上述筛选多在 **已拉取列表** 上做客户端过滤；数据量大时请后端为对应 `GET` 增加 query，并把筛选迁到服务端以保证分页 `total` 一致。
...
| 2026-03-30 | 扩容：配额/限流、熔断、健康检查、告警规则、分类、标签等列表检索或 CRUD 补全；`useMonitoring` 增加告警规则更新/删除 mutations。 |
| 2026-04-09 | 资源 Grant 管理页相关行改为已下线说明。 |
